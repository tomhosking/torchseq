...
class NgramReranker(nn.Module):
    ...
    def forward(self, candidates, lengths, batch, tgt_field, scores=None, sort=True, top1=True):
        ...
        refs_k_hot = (
            torch.sum(onehot(batch[self.src_field], N=self.config.prepro.vocab_size, ignore_index=BPE.pad_id), -2)
            .float()
            .unsqueeze(1)
        )

        candidates_k_hot = torch.sum(
            onehot(candidates, N=self.config.prepro.vocab_size, ignore_index=BPE.pad_id), -2
        ).float()
