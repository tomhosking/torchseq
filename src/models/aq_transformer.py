...
class TransformerAqModel(nn.Module):
    def __init__(self, config):
        ...
        self.encoder_decoder = nn.Transformer(d_model=config.embedding_dim+config.bio_embedding_dim,
        ...
        self.output_projection = nn.Linear(config.embedding_dim+config.bio_embedding_dim, config.vocab_size+1, bias=False)
        ...
        self.positional_embeddings = SinusoidalPositionalEmbedding(embedding_dim=config.embedding_dim+config.bio_embedding_dim, padding_idx=-1, init_size=400)
    ...
    def forward(self, batch, output):
        ...
        ctxt_embedded = torch.cat([ctxt_toks_embedded,ctxt_ans_embedded], dim=-1) + ctxt_pos_embeddings# + 
        output_embedded = torch.cat([output_embedded, output_ans_embeddings], dim=-1) + output_pos_embeddings
