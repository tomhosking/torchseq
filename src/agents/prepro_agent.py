...
class PreprocessorAgent(BaseAgent):
    def __init__(self, config):
        ...
        if self.config.training.dataset == 'squad':
            ...
            self.train = ParaphraseDataset(os.path.join(config.env.data_path, 'paranmt/'), config,  dev=False, test=False)
            self.valid = ParaphraseDataset(os.path.join(config.env.data_path, 'paranmt/'), config, dev=True, test=False)
            self.test = [] #SquadDataset(os.path.join(config.env.data_path, 'paranmt/'), config, dev=False, test=True)
