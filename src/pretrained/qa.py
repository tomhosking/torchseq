...
class PreTrainedQA:
    ...
    def infer_batch(self, question_list, context_list):
        ...
        input_ids_batch = [
            self.tokenizer.encode(question, text) for question, text in zip(question_list, context_list)
        ]
        token_type_ids_batch = [
            [0 if i <= input_ids.index(102) else 1 for i in range(len(input_ids))] for input_ids in input_ids_batch
        ]
        start_scores, end_scores = self.model(
            self.pad_batch(input_ids_batch), token_type_ids=self.pad_batch(token_type_ids_batch)
        )
        ...
        return [
            self.extract_answer(all_tokens_batch[ix], start_scores[ix], end_scores[ix], token_type_ids_batch[ix])
...
if __name__ == "__main__":
    ...
    print(
        instance.infer_batch(
            ...
            ["Jim Henson was a nice puppet", "Jim Henson was a nice puppet"],
        )
    )
