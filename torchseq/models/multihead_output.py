...
class MultiHeadOutput(nn.Module):
    def __init__(
        ...
        if num_projections > 1:
            self.embeds_to_logits = nn.ModuleList([nn.Linear(self.dim_per_head, vocab_size, bias=False) for _ in range(num_projections)]).cpu()
        else:
    ...
    def forward(self, embeds):
        if self.num_heads > 1 or self.num_projections > 1: 
            ...
            if self.num_projections > 1:
                ...
                logits_split = torch.cat([l(embeds_chunked) for l in self.embeds_to_logits], dim=2)
            else:
                with torch.no_grad():
