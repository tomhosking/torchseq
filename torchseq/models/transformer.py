...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        # pre-LN
        x = self.norm1(src)
        src2 = self.self_attn(x, x, x, attn_mask=src_mask, key_padding_mask=src_key_padding_mask)[0]
...
class TransformerDecoderLayer(Module):
    ...
    def forward(
        ...
        """
        x = self.norm1(tgt)
        tgt2 = self.self_attn(x, x, x, attn_mask=tgt_mask, key_padding_mask=tgt_key_padding_mask)[0]
        tgt = tgt + self.dropout1(tgt2)

        x = self.norm2(tgt)
        tgt2 = self.multihead_attn(x, memory, memory, attn_mask=memory_mask, key_padding_mask=memory_key_padding_mask)[
            0
        ]
        ...
        if hasattr(self, "activation"):
            ...
            tgt2 = self.linear2(self.dropout(F.relu(self.linear1(x))))
