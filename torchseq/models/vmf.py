...
class vMF(torch.nn.Module):
    def __init__(self, hid_dim, lat_dim, kappa=1):
        ...
        self.kld = torch.from_numpy(vMF._vmf_kld(kappa, lat_dim)).float()
    ...
    def _vmf_kld(k, d):
        tmp = (
            k * ((sp.iv(d / 2.0 + 1.0, k) + sp.iv(d / 2.0, k) * d / (2.0 * k)) / sp.iv(d / 2.0, k) - d / (2.0 * k))
            + d * np.log(k) / 2.0
            - np.log(sp.iv(d / 2.0, k))
            - sp.loggamma(d / 2 + 1)
            - d * np.log(2) / 2
        ).real
        if tmp != tmp:
    ...
    def _vmf_kld_davidson(k, d):
        ...
        """
        tmp = (
            k * sp.iv(d / 2, k) / sp.iv(d / 2 - 1, k)
            + (d / 2 - 1) * torch.log(k)
            - torch.log(sp.iv(d / 2 - 1, k))
            + np.log(np.pi) * d / 2
            + np.log(2)
            - sp.loggamma(d / 2).real
            - (d / 2) * np.log(2 * np.pi)
        )
        if tmp != tmp:
            exit()
    ...
    def build_bow_rep(self, lat_code, n_sample=1):
        ...
        kld = self.compute_KLD(tup, batch_sz).to(lat_code.device)
        vecs = []
        if n_sample == 1:
    ...
    def sample_cell(self, mu, norm, kappa):
        ...
        # batch version
        w_var = (w * torch.ones(batch_sz, lat_dim)).to(mu.device)
        v = self._sample_ortho_batch(mu, lat_dim)
        scale_factr = torch.sqrt((torch.ones(batch_sz, lat_dim).to(mu.device)) - torch.pow(w_var, 2))
