# mypy: ignore-errors
"""
    https://github.com/clementchadebec/benchmark_VAE/blob/main/src/pythae/models/pvae/pvae_utils.py
"""
...
class PoincareBall:
    ...
    def dist(self, x: torch.Tensor, y: torch.Tensor, *, keepdim=False, dim=-1) -> torch.Tensor:  ## OK  ## OK
        sqrt_c = self.c**0.5
        dist_c = artanh(sqrt_c * _mobius_add(-x, y, self.c, dim=dim).norm(dim=dim, p=2, keepdim=keepdim)).clamp_min(
            MIN_NORM
        )
    ...
    def mobius_add(self, x: torch.Tensor, y: torch.Tensor, *, dim=-1, project=True) -> torch.Tensor:  ## OK
    ...
    def logdetexp(self, x, y, is_vector=False, keepdim=False):  ## OK
        d = self.norm(x, y, keepdim=keepdim) if is_vector else self.dist(x, y, keepdim=keepdim) + 1e-10
        if (d < 0).any():
            print("Found negative norm!")
            exit()

        output = (torch.sinh(math.sqrt(self.c) * d) / math.sqrt(self.c) / d).log()
        if output.isnan().any():
            print("Found nan in logdetexp!")
            print(d)
            exit()
    ...
    def expmap_polar(self, x, u, r, dim: int = -1):  ## OK
        sqrt_c = self.c**0.5
        u_norm = u.norm(dim=dim, p=2, keepdim=True).clamp_min(MIN_NORM)
        second_term = tanh(torch.tensor([sqrt_c]).to(x.device) / 2 * r) * u / (sqrt_c * u_norm)
    ...
    if dim == 2:
        return (
            ...
            * (
                ...
                + torch.erf((value - math.sqrt(c) * scale.pow(2)) / math.sqrt(2) / scale)
        ...
        signs = torch.tensor([1.0, -1.0]).double().to(device).repeat(((int(dim) + 1) // 2) * 2)[: int(dim)]
...
def grad_cdf_value_scale(value, scale, c, dim):
    ...
    log_arg2 = math.sqrt(2 / math.pi) * (
        ...
        - (
            ...
            * torch.exp(-(value - (dim - 1 - 2 * k_float) * math.sqrt(c) * scale.pow(2)).pow(2) / (2 * scale.pow(2)))
        )
    )
    ...
    log_unormalised_prob = (
        -value.pow(2) / (2 * scale.pow(2)) + (dim - 1) * logsinh(math.sqrt(c) * value) - (dim - 1) / 2 * math.log(c)
    )
...
class _log_normalizer_closed_grad(Function):
    ...
    def forward(ctx, scale, c, dim):
        ...
        device = scale.device
        output = (
            0.5 * (math.log(math.pi) - math.log(2)) + scale.log() - (int(dim) - 1) * (math.log(c) / 2 + math.log(2))
        )
        dim = torch.tensor(int(dim)).to(device).double()
    ...
    def backward(ctx, grad_output):
        ...
        ) + torch.exp(-(dim - 1 - 2 * k_float).pow(2) * c * scale.pow(2) / 2) * 2 / math.sqrt(math.pi) * (
            dim - 1 - 2 * k_float
        ) * math.sqrt(
            c
        ) / math.sqrt(
...
class HyperbolicRadius(dist.Distribution):
    ...
    def grad_log_prob(self, value):
        res = -value / self.scale.pow(2) + (self.dim - 1) * math.sqrt(self.c) * torch.cosh(
            math.sqrt(self.c) * value
        ) / torch.sinh(math.sqrt(self.c) * value)
        return res
...
class HypersphericalUniform(dist.Distribution):
    ...
    def _log_surface_area(self):
        return (
            math.log(2) + ((self._dim + 1) / 2) * math.log(math.pi) - torch.lgamma(torch.Tensor([(self._dim + 1) / 2]))
        )
...
class GeodesicLayer(RiemannianLayer):
    ...
    def forward(self, input):
        ...
        input = input.unsqueeze(-2).expand(input.shape[0], self.out_features, self.in_features)
