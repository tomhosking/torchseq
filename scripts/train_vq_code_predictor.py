...
parser.add_argument(
 "--model_path", type=str, metavar="PATH", default='./runs/sep_ae/20201223_183223_vae_wa_6h_quantized_256_16qh/', help="Path to data folder"
)
parser.add_argument(
 "--output_path", type=str, metavar="PATH", default='./runs/mlpcodepredictor/', help="Path to data folder"
)
...
if not os.path.exists(MODEL_PATH+f'/sep_encoding_1_train.npy'):
    ...
    # _, _, _, memory_train = instance.validate(data_loader, use_train=True, save_model=False, memory_keys_to_return=['sep_encoding_1', 'sep_encoding_2','vq_codes'])
    ...
    # _, _, _, memory_dev = instance.validate(data_loader, use_train=False, save_model=False, memory_keys_to_return=['sep_encoding_1', 'sep_encoding_2','vq_codes'])
...
with jsonlines.open(os.path.join(args.data_dir, "wikianswers-pp/train.jsonl")) as f:
...
with jsonlines.open(os.path.join(args.data_dir, "wikianswers-pp/dev.jsonl")) as f:
...
if args.eval:
    ...
    with jsonlines.open(os.path.join(args.data_dir, "wikianswers-para-splitforgeneval/dev.jsonl")) as f:
    ...
    with jsonlines.open(args.data_dir + '/wikianswers-para-exemplarmlppredict/dev.jsonl', 'w') as f:
        ...
        for ix, row in enumerate(tqdm(rows)):
            ...
            probs, predicted = torch.topk(torch.softmax(outputs, -1), 6 -1)
    ...
    #         print(predicted.shape, probs.shape)
        ...
        #                 break
                ...
                joint_probs = sorted(joint_probs, key=lambda x: x[1], reverse=True)[:10]
        #     break

            gold = y_dev[ix]

        #         print(predicted, gold)
            pred_codes = [tgt_codes + x[0] for x in sorted(joint_probs, key=lambda x: x[1], reverse=True)[:5]]
    #         print(pred_codes)
